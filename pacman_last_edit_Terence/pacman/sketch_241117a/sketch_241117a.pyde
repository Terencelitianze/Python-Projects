...
vel = 5  # módulo da velocidade do pacman
...
text_array = []  # array da pontuação em String
...
def setup():
    ...
    # Tipo de letra
    crackmanFont = createFont(PATH + "/pacman/crackman.TTF", 18)
    lotFont = createFont(PATH + "/pacman/lot.otf", 18)
    
    # Ficheiros
    global text_array
    text_array = loadStrings(PATH + "/pacman/leaderboard.txt")
    
    # Música
    minim = Minim(this)
    intro = minim.loadFile(PATH + "/pacman/pacman_beginning.wav")
    minim.getLineOut().setGain(-12)
    chump = minim.loadFile(PATH + "/pacman/pacman_chomp.wav")
    minim.getLineOut().setGain(-20)
    death = minim.loadFile(PATH + "/pacman/pacman_death.wav")
    minim.getLineOut().setGain(-10)
    speedUp = minim.loadFile(PATH + "/pacman/pacman_speed.wav")
    minim.getLineOut().setGain(-12)
    god = minim.loadFile(PATH + "/pacman/pacman_eatfruit.wav")
    minim.getLineOut().setGain(-10)
    ghosts = minim.loadFile(PATH + "/pacman/pacman_ghosts.wav")
    minim.getLineOut().setGain(-12)
    silence = minim.loadFile(PATH + "/pacman/pacman_silence.wav")
...
def draw():
    global stage, px, py, score, speed, godmode, score
    
    if stage == "menu":
        img1 = loadImage(PATH + "/pacman/menu_inicial_1_original.jpg")
        image(img1, 0, 0)
        img2 = loadImage(PATH + "/pacman/pacman_letras.png")
        image(img2, 40, 80)
        buttonHighlight()
...
def desenharLabirinto():
    # Draw the grid lines first, ensuring they don't overlap obstacles
    fill(0)
    stroke(corFronteira)
    strokeWeight(espacamento)

    # Draw outer boundary of the maze
    rect(margemH, margemV, width - 2 * margemH, height - 2 * margemV)
    
    # Grid: Draw vertical and horizontal lines
    stroke(corFronteira)
    for i in range(nCol + 1):  # Ensure to draw grid up to the last column
        line(centroX(i), 0, centroX(i), height)  # Vertical lines
    for j in range(nLin + 1):  # Ensure to draw grid up to the last row
        line(0, centroY(j), width, centroY(j))  # Horizontal lines
    
    # Draw obstacles after the grid lines to ensure no overlap
    fill(corObstaculos)

    # Obstacles: Ensure obstacles are positioned correctly and inside the grid
    rect(centroX(1), centroY(1), tamanho * 3, tamanho)  # Horizontal obstacle
    rect(centroX(6), centroY(1), tamanho * 3, tamanho)  # Horizontal obstacle
    rect(centroX(1), centroY(3), tamanho, tamanho * 3)  # Vertical obstacle
    rect(centroX(3), centroY(3), tamanho * 3, tamanho)  # Horizontal obstacle
    rect(centroX(7), centroY(3), tamanho, tamanho * 3)  # Vertical obstacle
    rect(centroX(9), centroY(3), tamanho * 2, tamanho)  # Horizontal obstacle
    
    # You can continue adding more obstacles as needed in the grid.
...
def keyPressed():
    global direction, vx, vy, vel, pause, stage
...
def writeToFile():
    ...
    if isHighscore():
        ...
        with open(PATH + "/pacman/leaderboard.txt", "w") as f:
...
def readLeaderboard():
    ...
    with open(PATH + "/pacman/leaderboard.txt", "r") as f:
